...
query_run_main_method () {
  ...
  #step 2.  spark kmeans    :  Calculating k-means and write to hdfs result folder
...
#    fi
#
#    result_local_tmp=`mktemp -d`
#    echo "download results from ${RESULT_DIR} to ${result_local_tmp}"
#    hadoop fs -get "${RESULT_DIR}/*" ${result_local_tmp}
#    if diff -q "$VALIDATION_RESULTS_FILENAME" <(cat ${result_local_tmp}/*)
#    then
