...
ADD FILE ${QUERY_DIR}/q2-sessionize.py;
...
(
  ...
  REDUCE
    ...
    tstamp_inSec,
    wcs_item_sk
  USING 'python3 q2-sessionize.py ${q02_session_timeout_inSec}'
  AS (
    wcs_item_sk BIGINT,
    sessionid STRING)
) q02_tmp_sessionize
...
(
  ...
  (
    ...
    FROM ${TEMP_TABLE}
    GROUP BY sessionid
    HAVING array_contains(itemArray, cast(${q02_item_sk} AS BIGINT) ) -- eager filter out groups that don't contain the searched item
  ) collectedList
) pairs
